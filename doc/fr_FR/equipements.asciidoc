...
La ligne supérieure permet d'accéder à la page de <<Configuration du plugin>>, ou d'afficher l'état de santé du plugin.
Suit la section _Mes jMQTT_ qui affiche les équipements présents. Plusieurs remarques:
   
   - Un équipement grisé est inactif;
   - Un équipement cerclé d'une bordure rouge, ou grise si inactif, est un équipement dont l'_Ajout automatique des commandes_ est activé.
   

=== Création automatique des équipements

Le mode création automatique des équipements permet la découverte et la création automatique des équipements. Il s'active via la page de <<Configuration du plugin>>. Le plugin souscrit auprès du broker le topic configuré dans cette même page (# par défaut, i.e. tous les topics). A réception d'un message dont le topic de premier niveau est nouveau, le plugin crée automatiquement un équipement associé à ce topic de premier niveau.
...
TIP: Le mode création automatique des équipements n'influe que sur la création de l'équipement, pas sur la création des informations associées, qui dépend du paramètre _Ajout automatique des commandes_ que nous verrons dans le chapitre suivant.

IMPORTANT: Une fois les équipements découverts, il est conseillé de quitter le mode automatique pour éviter la création d'équipements non souhaités, notamment dans les situations suivantes : publication de messages (si jMQTT reste souscrit à tous les topics, il écoutera ses propres publications), essais avec le broker, tests de nouveaux équipements, ...
...
Lorsque Jeedom est en mode <<Création automatique des équipements>>, il créé un équipement, dont le nom est `_Identifiant de connexion_` qui a ce statut en information.
