...
/* TODO (medium) Implemented for later
	public static function brkRestart($brkId) {
		$params['cmd']      = 'brkRestart';
		$params['brkId']    = $brkId;
		jMQTTComToDaemon::send($params);
	}

	public static function incMode($brkId, $mode) {
		$params['cmd']      = 'incMode';
		$params['brkId']    = $brkId;
		$params['inc']      = $mode == 1;
		jMQTTComToDaemon::send($params);
	...
	public static function cfgChange($conf) {
		$params['cmd']      = 'cfgChange';
		$params['conf']     = $conf;
		jMQTTComToDaemon::send($params, false);
	}

	public static function delCmd($cmdId) {
		$params['cmd']      = 'delCmd';
		$params['cmdId']    = $cmdId;
		jMQTTComToDaemon::send($params, false);
	}

	public static function delEq($eqId) {
		$params['cmd']      = 'delEq';
		$params['eqId']     = $eqId;
		jMQTTComToDaemon::send($params, false);
	}
