...
<div class="row row-overflow" id='div_jmqttTemplate'>
  <div class="col-lg-3 col-md-4 col-sm-5" id="div_listJmqtt" style="z-index:999">
  ...
  <div class="col-lg-9 col-md-9 col-sm-7" id="div_listJmqttTemplate" style="display:none;">
    <form class="form-horizontal">
      <legend><i class="fas fa-home"></i> {{Général}}</legend>
      <a class="btn btn-sm btn-danger pull-right" id="bt_jmqttTemplateDelete"><i class="fas fa-times"></i> {{Supprimer}}</a>
      <!--<a class='btn btn-sm btn-success pull-right' id='bt_jmqttTemplateApply'><i class="far fa-check-circle"></i> {{Appliquer}}</a>-->
      <a class="btn btn-sm btn-primary pull-right" id="bt_jmqttTemplateDownload"><i class="fas fa-cloud-download-alt"></i> {{Télécharger}}</a>
      <br />
    ...
    callPluginAjax({
        ...
        success: function (dataresult) {
            ...
            for (var i in dataresult) {
                li += "<li class='cursor li_jmqttTemplate' data-name='" + i + "' data-template='" + JSON.stringify(dataresult[i]) + "'><a>" + i + "</a></li>"
            }
...
$('#ul_jmqttTemplateList').on({
  'click': function(event) {
    ...
    data = JSON.parse($(this).attr('data-template'))
...
//TODO Add more Equipment informations
        var eq =  '';
            ...
            eq += '<textarea class="eqLogicAttr form-control" data-l1key="configuration" data-l2key="commentaire" disabled>'+init(data.commentaire)+'</textarea>';
            eq += '</div></div>';
        ...
        for (var i in data['commands']) {
            ...
            if (init(_cmd.type) == 'info') {
                ...
                tr += '<td><textarea class="cmdAttr form-control input-sm" data-l1key="name" style="height:65px;" disabled placeholder="{{Nom de l\'info}}"></textarea></td>';
                tr += '<td>';
                ...
                tr += '</select></td><td>';
                tr += '<textarea class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="topic" disabled style="height:65px;" placeholder="{{Topic}}"></textarea>';
                tr += '</td><td>';
                tr += '</td><td>';
            ...
            if (init(_cmd.type) == 'action') {
                ...
                tr += '<td>';
                tr += '<div class="row">';
                tr += '<div class="col-sm-8"><input class="cmdAttr form-control input-sm" data-l1key="name" disabled></div>';
                tr += '</div>';
                ...
                tr += '</td><td>';
                tr += '<textarea class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="request" style="height:65px;" disabled placeholder="{{Valeur}}"></textarea>';
                tr += '</td><td>';
...
$('#bt_jmqttTemplateDelete').on('click', function() {
  if ($('#ul_jmqttTemplateList li.active').attr('data-name') == undefined) {
    ...
    if (result) {
      callPluginAjax({
        data: {
            ...
            template: $('#ul_jmqttTemplateList li.active').attr('data-name'),
        },
        error: function(error) {
            $('#md_jmqttTemplate').showAlert({message: error.message, level: 'danger'})
        },
...
$('#bt_jmqttTemplateDownload').on('click',function() {
    ...
    callPluginAjax({
        ...
        success: function (result) {
            window.open('core/php/downloadFile.php?pathfile=' + result, "_blank", null)
        }
    });
})
...
/* $('#div_jmqttTemplate').on({
  'click': function(event) {
    var el = $(this)
    jeedom.cmd.getSelectModal({}, function(result) {
      el.closest('.jmqttScenario').find('.jmqttScenarioAttr[data-l1key=end]').value(result.human)
    })
  }
}, '.bt_jmqttTemplateSelectCmd')
 */
</script>
