			...
			for (var i in data['commands']) {
				...
				if (init(_cmd.type) == 'action') {
					...
					if (init(_cmd.subType) == 'select')
						tr += '<input class="tooltips cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="listValue" placeholder="{{Liste : valeur|texte}}" disabled title="{{Liste : valeur|texte (séparées entre elles par des point-virgules)}}" placeholder="{{Liste de valeur|texte séparé par ;}}">';
					tr += '</td><td>';
