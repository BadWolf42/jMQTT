...
jmqtt.getMqttClientInfo = function(_eq) {
    ...
    if (_eq.isEnable == '1')
        return {la: 'ok',  lacolor: 'success', state: 'nok', message: "{{Le Démon jMQTT n'arrive pas à se connecter à ce Broker}}", color:'danger'};
    // Client is disabled
    return     {la: 'nok', lacolor: 'danger',  state: 'nok', message: "{{La connexion à ce Broker est désactivée}}",                color:'warning'};
}
