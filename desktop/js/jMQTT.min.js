...
$jscomp.polyfill("String.prototype.repeat",function(a){return a?a:function(a){var b=$jscomp.checkStringArgs(this,null,"repeat");if(0>a||1342177279<a)throw new RangeError("Invalid count value");a|=0;for(var c="";a;)if(a&1&&(c+=b),a>>>=1)b+=b;return c}},"es6","es3");var refreshTimeout,N_CMD;
function initPluginUrl(a){a=void 0===a?["id","saveSuccessFull","removeSuccessFull"]:a;var c=getUrlVars(),b="",d;for(d in c)0>$.inArray(d,a)&&(0<b.length&&(b+="&"),b+=d+"="+c[d].replace("#",""));return"index.php?"+b}
function refreshEqLogicPage(){function a(){void 0!=$(".li_eqLogic.active").attr("data-eqLogic_id")?$(".li_eqLogic[data-eqLogic_id="+$(".li_eqLogic.active").attr("data-eqLogic_id")+"]").click():$(".eqLogicAction[data-action=returnToThumbnailDisplay]").click()}modifyWithoutSave?bootbox.confirm("{{La page a \u00e9t\u00e9 modifi\u00e9e. Etes-vous s\u00fbr de vouloir la recharger sans sauver ?}}",function(c){c&&a()}):a()}$(document).ready(function(){window.location.href.includes("#commandtab")&&$('a[href="#commandtab"]').click()});
...
$("#bt_json").on("click",function(){refreshEqLogicPage();$("#bt_json").removeClass("btn-default").addClass("btn-primary");$("#bt_classic").removeClass("btn-primary").addClass("btn-default")});$('a[href="#eqlogictab"],a[href="#brokertab"]').on("click",function(){$("#menu-bar").hide()});$('a[href="#commandtab"]').on("click",function(){$("#menu-bar").show()});
$(".li_eqLogic").on("click",function(){var a=getUrlVars("id"),c=$(this).attr("data-eqLogic_id");is_numeric(a)&&a==c||history.pushState({},"",initPluginUrl()+"&id="+c)});$(".eqLogicAction[data-action=returnToThumbnailDisplay]").on("click",function(){history.pushState({},"",initPluginUrl())});1==getUrlVars("saveSuccessFull")&&($("#div_alert").showAlert({message:"{{Sauvegarde effectu\u00e9e avec succ\u00e8s}}",level:"success"}),history.replaceState({},"",initPluginUrl(["saveSuccessFull"])));
1==getUrlVars("removeSuccessFull")&&($("#div_alert").showAlert({message:"{{Suppression effectu\u00e9e avec succ\u00e8s}}",level:"success"}),history.replaceState({},"",initPluginUrl(["removeSuccessFull"])));$("#table_cmd").sortable({axis:"y",cursor:"move",items:".cmd",placeholder:"ui-state-highlight",tolerance:"intersect",forcePlaceholderSize:!0});
$(".eqLogicAction[data-action=addBroker]").on("click",function(){bootbox.prompt("{{Nom du broker ?}}",function(a){null!==a&&jeedom.eqLogic.save({type:eqType,eqLogics:[{name:a,type:"broker"}],error:function(a){$("#div_alert").showAlert({message:a.message,level:"danger"})},success:function(a){var b=initPluginUrl();modifyWithoutSave=!1;b+="&id="+a.id+"&saveSuccessFull=1";loadPage(b)}})})});
function printEqLogic(a){function c(a){a=parseInt(a).toString(36);3>a.length&&(a="0".repeat(3-a.length)+a);return a}function b(d){if(void 0!=d.sOrder)return d.sOrder;var e="";void 0!=d.configuration.jParent&&0<=d.configuration.jParent&&(e=a.cmd.filter(function(a){return a.id==d.configuration.jParent}),e=b(e[0]));sOrder=void 0==d.configuration.jOrder||0>d.configuration.jOrder?c(d.order):c(d.configuration.jOrder);return e+sOrder}if($("#bt_json.active").length){N_CMD=1;for(var d=$jscomp.makeIterator(a.cmd),
e=d.next();!e.done;e=d.next())e=e.value,e.sOrder=b(e);a.cmd.sort(function(a,b){return a.sOrder<b.sOrder?-1:a.sOrder>b.sOrder?1:0});$("#table_cmd").sortable("disable");$("#table_cmd th:first").width("120px")}else $("#table_cmd").sortable("enable"),$("#table_cmd th:first").width("50px");"broker"==a.configuration.type?($(".typ-std").hide(),$(".typ-brk").show()):($(".typ-brk").hide(),$(".typ-std").show())}
function addCmdToTable(a){isset(a)||(a={configuration:{}});isset(a.configuration)||(a.configuration={});if("info"==init(a.type)){var c="1"==init(a.configuration.virtualAction)?"disabled":"",b='<tr class="cmd';$("#bt_json.active").length&&(b+=" treegrid-"+N_CMD,0<=a.configuration.jParent&&(b+=" treegrid-parent-"+$(".cmd[data-cmd_id="+a.configuration.jParent+"]").attr("class").split("treegrid-")[1]));b+='" data-cmd_id="'+init(a.id)+'">';b+='<td><span class="cmdAttr" data-l1key="id"></span>';b=$("#bt_json.active").length?
b+'<span style="display:none;" class="cmdAttr" data-l1key="order"></span></td>':b+"</td>";b+='<td><textarea class="cmdAttr form-control input-sm" data-l1key="name" style="height:65px;" placeholder="{{Nom de l\'info}}" /></td>';b+="<td>";b+='<input class="cmdAttr form-control type input-sm" data-l1key="type" value="info" disabled style="margin-bottom:5px;width:120px;" />';b+='<span class="cmdAttr subType" subType="'+init(a.subType)+'"></span>';b+="</td><td>";b+='<textarea class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="topic" style="height:65px;" '+
c+' placeholder="{{Topic}}" readonly=true />';b+="</td><td>";b+='<textarea class="cmdAttr form-control input-sm" data-key="value" style="height:65px;" '+c+' placeholder="{{Valeur}}" readonly=true />';b+="</td><td>";b+='<input class="cmdAttr form-control input-sm" data-l1key="unite" placeholder="{{Unit\u00e9}}"></td><td>';b+='<span><label class="checkbox-inline"><input type="checkbox" class="cmdAttr checkbox-inline" data-l1key="isHistorized" checked/>{{Historiser}}</label></span> ';b+='<span><label class="checkbox-inline"><input type="checkbox" class="cmdAttr checkbox-inline" data-l1key="isVisible" checked/>{{Afficher}}</label></span> ';
...
b+='<i class="fa fa-minus-circle pull-right cmdAction cursor" data-action="remove"></i>';b+='<input style="width:82%;margin-bottom:2px;" class="tooltips cmdAttr form-control input-sm" data-l1key="cache" data-l2key="lifetime" placeholder="{{Lifetime cache}}" title="{{Lifetime cache}}">';b+='<input class="tooltips cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="minValue" placeholder="{{Min}}" title="{{Min}}" style="width:40%;display:inline-block;"> ';b+='<input class="tooltips cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="maxValue" placeholder="{{Max}}" title="{{Max}}" style="width:40%;display:inline-block;">';
b+="</td></tr>";$("#table_cmd tbody").append(b);$("#table_cmd tbody tr:last").setValues(a,".cmdAttr");isset(a.type)&&$("#table_cmd tbody tr:last .cmdAttr[data-l1key=type]").value(init(a.type));jeedom.cmd.changeType($("#table_cmd tbody tr:last"),init(a.subType));jeedom.cmd.execute({id:a.id,cache:0,notify:!1,success:function(b){$(".cmd[data-cmd_id="+a.id+"] .form-control[data-key=value]").value(b)}});jeedom.cmd.update[a.id]=function(b){$(".cmd[data-cmd_id="+a.id+"] .form-control[data-key=value]").value(b.display_value)};
N_CMD++}"action"==init(a.type)&&(b='<tr class="cmd treegrid-'+N_CMD+'" data-cmd_id="'+init(a.id)+'">',b+="<td>",b+='<span class="cmdAttr" data-l1key="id"></span>',b+="</td>",b+="<td>",b+='<div class="row">',b+='<div class="col-sm-4">',b+='<a class="cmdAction btn btn-default btn-sm" data-l1key="chooseIcon" style="padding-left:5px;padding-right:5px;"><i class="fa fa-flag"></i>  Ic\u00f4ne</a>',b+='<span class="cmdAttr" data-l1key="display" data-l2key="icon" style="margin-left:5px;"></span>',b+="</div>",
b+='<div class="col-sm-8">',b+='<input class="cmdAttr form-control input-sm" data-l1key="name">',b+="</div>",b+="</div>",b+='<select class="cmdAttr form-control tooltips input-sm" data-l1key="value" style="display:none;margin-top:5px;margin-right:10px;" title="{{Valeur par d\u00e9faut de la commande}}">',b+='<option value="">Aucune</option>',b+="</select>",b+="</td>",b+="<td>",b+='<input class="cmdAttr form-control type input-sm" data-l1key="type" value="action" disabled style="margin-bottom:5px;width:120px;" />',
