...
$jscomp.findInternal=function(a,c,d){a instanceof String&&(a=String(a));for(var e=a.length,b=0;b<e;b++){var f=a[b];if(c.call(d,f,b,a))return{i:b,v:f}}return{i:-1,v:void 0}};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(a,c,d){a!=Array.prototype&&a!=Object.prototype&&(a[c]=d.value)};
...
$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(a,d){return $jscomp.findInternal(this,a,d).v}},"es6","es3");var refreshTimeout;window.addEventListener("popstate",function(a){null==a.state&&location.reload(!1)});
function callPluginAjax(a){$.ajax({async:void 0==a.async?!0:a.async,global:!1,type:"POST",url:"plugins/jMQTT/core/ajax/jMQTT.ajax.php",data:a.data,dataType:"json",error:function(a,d,e){handleAjaxError(a,d,e)},success:function(c){"ok"!=c.state?$("#div_alert").showAlert({message:c.result,level:"danger"}):"function"===typeof a.success&&a.success(c.result)}})}
function initPluginUrl(a,c,d){a=void 0===a?["id","saveSuccessFull","removeSuccessFull","hash"]:a;c=void 0===c?"":c;d=void 0===d?"":d;var e=getUrlVars(),b="index.php?",f;for(f in e)0>$.inArray(f,a)&&("?"!=b.substr(-1)&&(b+="&"),b+=f+"="+e[f].replace("#",""));""!=c&&(b+="&id="+c);""!=document.location.hash&&0>$.inArray("hash",a)&&(b+=document.location.hash);""!=d&&(b+=d);return b}
...
$('.nav-tabs a[role="tab"]').on("click",function(){document.location.hash!=$(this)[0].hash&&(url=initPluginUrl(["hash"],"",$(this)[0].hash),""==document.location.hash&&"#eqlogictab"==$(this)[0].hash?history.replaceState({hash:$(this)[0].hash},"",url):history.pushState({hash:$(this)[0].hash},"",url))});
$(".li_eqLogic,.eqLogicDisplayCard").on("click",function(){var a=getUrlVars("id"),c=$(this).attr("data-eqLogic_id"),d=document.location.hash;is_numeric(a)&&a==c||("#brokertab"==d&&"broker"!=$(this).attr("jmqtt_type")&&(d=""),url=initPluginUrl(["id","hash"],c,d),history.pushState({},"",url))});$(".eqLogicAction[data-action=returnToThumbnailDisplay]").on("click",function(){url=initPluginUrl();history.pushState({},"",url)});
...
("broker"==c?"{{le broker}}":"{{l'\u00e9quipement}}")+" <b>"+$(".eqLogicAttr[data-l1key=name]").value()+"</b> ?",function(d){d&&("broker"==c?bootbox.confirm('<table><tr><td style="vertical-align:middle;font-size:2em;padding-right:10px"><span class="label label-warning"><i class="fa fa-warning"</i></span></td><td style="vertical-align:middle">{{Tous les \u00e9quipements associ\u00e9s au broker vont \u00eatre supprim\u00e9s}}...<br><b>{{\u00cates vous s\u00fbr ?}}</b></td></tr></table>',function(c){c&&
a()}):a())})}else $("#div_alert").showAlert({message:"{{Veuillez d'abord s\u00e9lectionner un}} "+eqType,level:"danger"})});
$(".eqLogicAction[data-action=move_broker]").on("click",function(){var a=$(".eqLogicAttr[data-l1key=id]").value(),c=$("#broker").val();void 0!=a&&void 0!=c&&bootbox.confirm('<table><tr><td style="vertical-align:middle;font-size:2em;padding-right:10px"><span class="label label-warning"><i class="fa fa-warning"</i></span></td><td style="vertical-align:middle">{{Vous \u00eates sur le point de changer l\'\u00e9quipement de broker}}.<br>{{\u00cates vous s\u00fbr ?}}</td></tr></table>',function(d){d&&callPluginAjax({async:!1,
data:{action:"moveToBroker",id:a,brk_id:c},success:function(a){window.location.reload()}})})});
function printEqLogic(a){var c=1;if(0!=$("#bt_json.active").length){for(var d=function(a,b,f,g){(void 0===g?0:g)&&e(a,b,void 0===f?-1:f);f=c-1;for(i in b)"object"===typeof b[i]?d(a+"{"+i+"}",b[i],f,!0):e(a+"{"+i+"}",b[i],f)},e=function(c,d,f){var e="object"===typeof d?JSON.stringify(d):d,g=a.cmd.filter(function(a){return a.configuration.topic==c});0<g.length?(g[0].value=e,b(g[0],f)):b({configuration:{topic:c},isHistorized:"0",isVisible:"1",json:d,type:"info",subType:"string",value:e},f)},b=function(a,
b){a.treegrid_id=c++;0<b&&(a.treegrid_parent_id=b);f.push(a)},f=[],k=$jscomp.makeIterator(a.cmd),h=k.next();!h.done;h=k.next()){var g=h.value;if(!(0<=g.configuration.topic.search("{"))&&(b(g),"info"==g.type)){jeedom.cmd.execute({async:!1,id:g.id,cache:0,notify:!1,success:function(a){g.value=a}});try{g.json=JSON.parse(g.value)}catch(m){}"object"===typeof g.json&&d(g.configuration.topic,g.json)}}a.cmd=f;$("#table_cmd").sortable("disable")}else{k=$jscomp.makeIterator(a.cmd);for(h=k.next();!h.done;h=
k.next())g=h.value,g.treegrid_id=c++;$("#table_cmd").sortable("enable")}void 0==a.configuration.brkId||0>a.configuration.brkId||"eqpt"!=a.configuration.type&&"broker"!=a.configuration.type?($(".toDisable").addClass("disabled"),$(".typ-brk").hide(),$(".typ-std").show()):"broker"==a.configuration.type?($(".toDisable").removeClass("disabled"),$(".typ-std").hide(),$(".typ-brk").show(),$("#mqtttopic").prop("readonly",!0),h="jMQTT_"+(a.name.replace(" ","_")||"jeedom"),$("input[name=rd_logupdate]").attr("data-l1key",
"log::level::"+h),$(".bt_plugin_conf_view_log").attr("data-log",h),$(".bt_plugin_conf_view_log").html('<i class="fa fa fa-file-text-o"></i> '+h),refreshDaemonInfo(),jeedom.config.load({configuration:$("#div_broker_log").getValues(".configKey")[0],plugin:"jMQTT",error:function(a){$("#div_alert").showAlert({message:a.message,level:"danger"})},success:function(a){$("#div_broker_log").setValues(a,".configKey")}})):"eqpt"==a.configuration.type&&($(".toDisable").removeClass("disabled"),$(".typ-brk").hide(),
$(".typ-std").show(),$("#mqtttopic").prop("readonly",!1));var l=$("#broker");l.empty();$.each(eqBrokers,function(a,b){l.append(new Option(b,a))});l.val(a.configuration.brkId)}function saveEqLogic(a){var c=$("#div_broker_log").getValues(".configKey")[0];$.isEmptyObject(c)||(a.loglevel=c);for(c=a.cmd.length-1;0<=c;c--)""==a.cmd[c].id&&""==a.cmd[c].name&&a.cmd.splice(c,1);return a}
function addCmdToTable(a){isset(a)||(a={configuration:{}});isset(a.configuration)||(a.configuration={});var c=0!=$("#bt_json.active").length;if("info"==init(a.type)){var d=function(b){$(".treegrid-"+a.treegrid_id+" .form-control[data-key=value]").value(b)},e="1"==init(a.configuration.virtualAction)?"disabled":"",b='<tr class="cmd treegrid-'+a.treegrid_id;c&&0<a.treegrid_parent_id&&(b+=" treegrid-parent-"+a.treegrid_parent_id);b+='" data-cmd_id="'+init(a.id)+'">';b+='<td class="fitwidth"><span class="cmdAttr" data-l1key="id"></span>';
b=c?b+'<span style="display:none;" class="cmdAttr" data-l1key="order"></span></td>':b+"</td>";b+='<td><textarea class="cmdAttr form-control input-sm" data-l1key="name" style="height:65px;" placeholder="{{Nom de l\'info}}" /></td>';b+="<td>";b+='<input class="cmdAttr form-control type input-sm" data-l1key="type" value="info" disabled style="margin-bottom:5px;width:120px;" />';b+='<span class="cmdAttr subType" subType="'+init(a.subType)+'"></span>';b+="</td><td>";b+='<textarea class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="topic" style="height:65px;" '+
e+' placeholder="{{Topic}}" readonly=true />';b+="</td><td>";b+='<textarea class="form-control input-sm" data-key="value" style="height:65px;" '+e+' placeholder="{{Valeur}}" readonly=true />';b+='</td><td class="fitwidth">';b+='<input class="cmdAttr form-control input-sm" data-l1key="unite" placeholder="{{Unit\u00e9}}"></td><td>';b+='<span><label class="checkbox-inline"><input type="checkbox" class="cmdAttr checkbox-inline" data-l1key="isHistorized" checked/>{{Historiser}}</label></span> ';b+='<span><label class="checkbox-inline"><input type="checkbox" class="cmdAttr checkbox-inline" data-l1key="isVisible" checked/>{{Afficher}}</label></span> ';
b+='<span><label class="checkbox-inline"><input type="checkbox" class="cmdAttr checkbox-inline" data-l1key="display" data-l2key="invertBinary"/>{{Inverser}}</label></span> ';b+='<span><label class="checkbox-inline"><input type="checkbox" class="cmdAttr checkbox-inline" data-l1key="configuration" data-l2key="parseJson"/>{{parseJson}}</label></span> ';b+="</td>";b+="<td>";is_numeric(a.id)&&(b+='<a class="btn btn-default btn-xs cmdAction" data-action="configure"><i class="fa fa-cogs"></i></a> ',b+='<a class="btn btn-default btn-xs cmdAction" data-action="test"><i class="fa fa-rss"></i> {{Tester}}</a>');
void 0!=a.id&&void 0==a.configuration.irremovable&&(b+=' <a class="btn btn-default btn-xs cmdAction pull-right" data-action="remove"><i class="fa fa-minus-circle"></i></a>');b+="</td></tr>";$("#table_cmd tbody").append(b);$("#table_cmd tbody tr:last").setValues(a,".cmdAttr");isset(a.type)&&$("#table_cmd tbody tr:last .cmdAttr[data-l1key=type]").value(init(a.type));jeedom.cmd.changeType($("#table_cmd tbody tr:last"),init(a.subType));d(a.value);void 0!=a.id&&(c||jeedom.cmd.execute({id:a.id,cache:0,
notify:!1,success:function(a){d(a)}}),jeedom.cmd.update[a.id]=function(a){d(a.display_value)})}"action"==init(a.type)&&(b='<tr class="cmd treegrid-'+a.treegrid_id+'" data-cmd_id="'+init(a.id)+'">',b+="<td>",b+='<span class="cmdAttr" data-l1key="id"></span>',b+="</td>",b+="<td>",b+='<div class="row">',b+='<div class="col-sm-4">',b+='<a class="cmdAction btn btn-default btn-sm" data-l1key="chooseIcon" style="padding-left:5px;padding-right:5px;"><i class="fa fa-flag"></i>  Ic\u00f4ne</a>',b+='<span class="cmdAttr" data-l1key="display" data-l2key="icon" style="margin-left:5px;"></span>',
b+="</div>",b+='<div class="col-sm-8">',b+='<input class="cmdAttr form-control input-sm" data-l1key="name">',b+="</div>",b+="</div>",b+='<select class="cmdAttr form-control tooltips input-sm" data-l1key="value" style="display:none;margin-top:5px;margin-right:10px;" title="{{Valeur par d\u00e9faut de la commande}}">',b+='<option value="">Aucune</option>',b+="</select>",b+="</td>",b+="<td>",b+='<input class="cmdAttr form-control type input-sm" data-l1key="type" value="action" disabled style="margin-bottom:5px;width:120px;" />',
b+='<span class="cmdAttr subType" subType="'+init(a.subType)+'" style=""></span>',b+="</td>",b+="<td>",b+='<textarea class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="topic" style="height:65px;"'+e+' placeholder="{{Topic}}"></textarea><br/>',b+="</td><td>",b+='<textarea class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="request" style="height:30px;" '+e+' placeholder="{{Valeur}}"></textarea>',b+='<a class="btn btn-default btn-sm cursor listEquipementInfo" data-input="request" style="margin-top:5px;margin-left:5px;"><i class="fa fa-list-alt "></i> {{Rechercher \u00e9quipement}}</a>',
...
b+="</td>",b+="<td>",is_numeric(a.id)&&(b+='<a class="btn btn-default btn-xs cmdAction expertModeVisible" data-action="configure"><i class="fa fa-cogs"></i></a> ',b+='<a class="btn btn-default btn-xs cmdAction" data-action="test"><i class="fa fa-rss"></i> {{Tester}}</a>'),b+='<a class="btn btn-default btn-xs cmdAction pull-right" data-action="remove"><i class="fa fa-minus-circle"></i></a></td>',b+="</tr>",$("#table_cmd tbody").append(b),b=$("#table_cmd tbody tr:last"),jeedom.eqLogic.builSelectCmd({id:$(".li_eqLogic.active").attr("data-eqLogic_id"),
filter:{type:"info"},error:function(a){$("#div_alert").showAlert({message:a.message,level:"danger"})},success:function(c){b.find(".cmdAttr[data-l1key=value]").append(c);b.setValues(a,".cmdAttr");jeedom.cmd.changeType(b,init(a.subType))}}));c&&$(".tree").treegrid({initialState:"collapsed"})}
$("body").off("jMQTT::cmdAdded").on("jMQTT::cmdAdded",function(a,c){a=0==$("#div_newCmdMsg.alert").length?"{{La commande}} <b>"+c.cmd_name+"</b> {{est ajout\u00e9e \u00e0 l'\u00e9quipement}} <b>"+c.eqlogic_name+"</b>.":"{{Plusieurs commandes sont ajout\u00e9es \u00e0 l'\u00e9quipement}} <b>"+c.eqlogic_name+"</b>.";modifyWithoutSave||$(".li_eqLogic.active").attr("data-eqLogic_id")!=c.eqlogic_id||0!=$('div[role="dialog"]').filter(":visible").length||!c.reload?$("#div_newCmdMsg").showAlert({message:a,
