...
$jscomp.polyfill("String.prototype.repeat",function(a){return a?a:function(a){var c=$jscomp.checkStringArgs(this,null,"repeat");if(0>a||1342177279<a)throw new RangeError("Invalid count value");a|=0;for(var b="";a;)if(a&1&&(b+=c),a>>>=1)c+=c;return b}},"es6","es3");var refreshTimeout,N_CMD;window.addEventListener("popstate",function(a){null==a.state&&location.reload(!1)});
...
$(".eqLogicAction[data-action=runDev]").on("click",function(){$.ajax({type:"POST",url:"plugins/jMQTT/core/ajax/jMQTT.ajax.php",data:{action:"dev"},dataType:"json",error:function(a,c,d){handleAjaxError(a,c,d)},success:function(a){"ok"!=a.state&&$("#div_alert").showAlert({message:a.result,level:"danger"})}})});
...
$(".eqLogicAction[data-action=remove_jmqtt]").on("click",function(){"undefined"===typeof $(this).attr("brkId")?void 0!=$(".eqLogicAttr[data-l1key=id]").value()?bootbox.confirm("{{Etes-vous s\u00fbr de vouloir supprimer le broker}} <b>"+$(".eqLogicAttr[data-l1key=name]").value()+"</b> ?",function(a){a&&bootbox.confirm('<table><tr><td style="vertical-align:middle;font-size:2em;padding-right:10px"><span class="label label-warning"><i class="fa fa-warning"</i></span></td><td style="vertical-align:middle">{{Tous les \u00e9quipements associ\u00e9s au broker vont \u00eatre supprim\u00e9s}}...<br><b>{{\u00cates vous s\u00fbr ?}}</b></td></tr></table>',function(a){a&&
jeedom.eqLogic.remove({type:eqType,id:$(".eqLogicAttr[data-l1key=id]").value(),error:function(a){$("#div_alert").showAlert({message:a.message,level:"danger"})},success:function(){var a=initPluginUrl();modifyWithoutSave=!1;a+="&id="+data.id+"&removeSuccessFull=1";loadPage(a)}})})}):$("#div_alert").showAlert({message:"{{Veuillez d'abord s\u00e9lectionner un}} "+eqType,level:"danger"}):$(this).click()});
...
e=b.next();!e.done;e=b.next())e=e.value,e.sOrder=d(e);a.cmd.sort(function(a,b){return a.sOrder<b.sOrder?-1:a.sOrder>b.sOrder?1:0});$("#table_cmd").sortable("disable");$("#table_cmd th:first").width("120px")}else $("#table_cmd").sortable("enable"),$("#table_cmd th:first").width("50px");"broker"==a.configuration.type?($(".typ-std").hide(),$(".typ-brk").show(),$("#sel_icon_div").css("visibility","hidden"),$("#mqtttopic").prop("readonly",!0),b="jMQTT_"+(a.configuration.mqttId||"jeedom"),$(".bt_plugin_conf_view_log").attr("data-log",
b),$(".bt_plugin_conf_view_log").html('<i class="fa fa fa-file-text-o"></i> '+b)):($(".typ-brk").hide(),$(".typ-std").show(),$("#sel_icon_div").css("visibility","visible"),$("#mqtttopic").prop("readonly",!1))}
...
function configureIncludeModeDisplay(a,c){1==c?($(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"]:not(.card)").removeClass("btn-default").addClass("btn-success"),$(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"]").attr("data-mode",1),$(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"].card span center").text("{{Arr\u00eater l'inclusion}}"),$(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"]:not(.card)").html('<i class="fa fa-sign-in fa-rotate-90"></i> {{Arreter inclusion}}'),
$(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"]").addClass("include"),$("#div_inclusionModeMsg").showAlert({message:"{{Mode inclusion automatique pendant 2 \u00e0 3min. Cliquez sur le bouton pour forcer la sortie de ce mode avant.}}",level:"warning"})):($(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"]:not(.card)").addClass("btn-default").removeClass("btn-success btn-danger"),$(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"]").attr("data-mode",0),$(".eqLogicAction[data-action=changeIncludeMode][brkId="+
a+"]:not(.card)").html('<i class="fa fa-sign-in fa-rotate-90"></i> {{Mode inclusion}}'),$(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"].card span center").text("{{Mode inclusion}}"),$(".eqLogicAction[data-action=changeIncludeMode][brkId="+a+"]").removeClass("include"),$("#div_inclusionModeMsg").hideAlert())}
$(".eqLogicAction[data-action=changeIncludeMode]").on("click",function(){var a=$(this);console.log(a.attr("data-mode"));1==a.attr("data-mode")?configureIncludeModeDisplay(a.attr("brkId"),0):configureIncludeModeDisplay(a.attr("brkId"),1);$.ajax({type:"POST",url:"plugins/jMQTT/core/ajax/jMQTT.ajax.php",data:{action:"changeIncludeMode",mode:a.attr("data-mode"),id:a.attr("brkId")},dataType:"json",error:function(a,d,b){handleAjaxError(a,d,b)},success:function(a){"ok"!=a.state&&$("#div_alert").showAlert({message:a.result,
level:"danger"})}})});$("body").off("jMQTT::disableIncludeMode").on("jMQTT::disableIncludeMode",function(a,c){configureIncludeModeDisplay(c.brkId,0)});
