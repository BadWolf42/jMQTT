...
$(document).ready(function(){document.location.toString().match("#")&&$('.nav-tabs a[href="#'+url.split("#")[1]+'"]').click();history.replaceState({},"",url)});$("#bt_addMQTTInfo").on("click",function(a){addCmdToTable({type:"info"});modifyWithoutSave=!0});$("#bt_addMQTTAction").on("click",function(a){addCmdToTable({type:"action"});modifyWithoutSave=!0});$("#bt_healthMQTT").on("click",function(){$("#md_modal").dialog({title:"{{Sant\u00e9 jMQTT}}"});$("#md_modal").load("index.php?v=d&plugin=jMQTT&modal=health").dialog("open")});
...
$("#bt_json").on("click",function(){refreshEqLogicPage();$("#bt_json").removeClass("btn-default").addClass("btn-primary");$("#bt_classic").removeClass("btn-primary").addClass("btn-default")});$('.nav-tabs a[href="#eqlogictab"]').on("click",function(){$("#menu-bar").hide()});$('.nav-tabs a[href="#commandtab"]').on("click",function(){$("#menu-bar").show()});
$('.nav-tabs a[role="tab"]').on("click",function(){document.location.hash!=$(this)[0].hash&&(url=initPluginUrl(["hash"],"",$(this)[0].hash),""==document.location.hash&&"#eqlogictab"==$(this)[0].hash?history.replaceState({hash:$(this)[0].hash},"",url):history.pushState({hash:$(this)[0].hash},"",url))});$(".li_eqLogic,.eqLogicDisplayCard").on("click",function(){var a=getUrlVars("id"),c=$(this).attr("data-eqLogic_id");is_numeric(a)&&a==c||(url=initPluginUrl(["id"],c),history.pushState({},"",url))});
$(".eqLogicAction[data-action=returnToThumbnailDisplay]").on("click",function(){url=initPluginUrl();history.pushState({},"",url)});1==getUrlVars("saveSuccessFull")&&($("#div_alert").showAlert({message:"{{Sauvegarde effectu\u00e9e avec succ\u00e8s}}",level:"success"}),history.replaceState({},"",initPluginUrl(["saveSuccessFull"])));1==getUrlVars("removeSuccessFull")&&($("#div_alert").showAlert({message:"{{Suppression effectu\u00e9e avec succ\u00e8s}}",level:"success"}),history.replaceState({},"",initPluginUrl(["removeSuccessFull"])));
...
e=b.next();!e.done;e=b.next())e=e.value,e.sOrder=d(e);a.cmd.sort(function(a,b){return a.sOrder<b.sOrder?-1:a.sOrder>b.sOrder?1:0});$("#table_cmd").sortable("disable");$("#table_cmd th:first").width("120px")}else $("#table_cmd").sortable("enable"),$("#table_cmd th:first").width("50px")}
...
function configureIncludeModeDisplay(a){1==a?($(".bt_changeIncludeMode:not(.card)").removeClass("btn-default").addClass("btn-success"),$(".bt_changeIncludeMode").attr("data-mode",1),$(".bt_changeIncludeMode.card span").text("{{Arr\u00eater l'inclusion}}"),$(".bt_changeIncludeMode").addClass("include"),$("#div_inclusionModeMsg").showAlert({message:"{{Mode inclusion automatique pendant 2 \u00e0 3min. Cliquez sur le bouton pour forcer la sortie de ce mode avant.}}",level:"warning"})):($(".bt_changeIncludeMode:not(.card)").addClass("btn-default").removeClass("btn-success btn-danger"),
$(".bt_changeIncludeMode").attr("data-mode",0),$(".bt_changeIncludeMode.card span").text("{{Mode inclusion}}"),$(".bt_changeIncludeMode").removeClass("include"),$("#div_inclusionModeMsg").hideAlert())}
$(".bt_changeIncludeMode").on("click",function(){var a=$(this);1==a.attr("data-mode")?configureIncludeModeDisplay(0):configureIncludeModeDisplay(1);$.ajax({type:"POST",url:"plugins/jMQTT/core/ajax/jMQTT.ajax.php",data:{action:"setIncludeMode",state:a.attr("data-mode")},dataType:"json",error:function(a,d,b){handleAjaxError(a,d,b)},success:function(a){"ok"!=a.state&&$("#div_alert").showAlert({message:a.result,level:"danger"})}})});
