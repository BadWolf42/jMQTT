...
function callPluginAjax(a){$.ajax({async:void 0==a.async?!0:a.async,global:!1,type:"POST",url:"plugins/jMQTT/core/ajax/jMQTT.ajax.php",data:a.data,dataType:"json",error:function(a,d,b){handleAjaxError(a,d,b)},success:function(c){"ok"!=c.state?$("#div_alert").showAlert({message:c.result,level:"danger"}):"function"===typeof a.success&&a.success(c.result)}})}
...
e=b.next();!e.done;e=b.next())e=e.value,e.sOrder=d(e);a.cmd.sort(function(a,b){return a.sOrder<b.sOrder?-1:a.sOrder>b.sOrder?1:0});$("#table_cmd").sortable("disable");$("#table_cmd th:first").width("120px")}else $("#table_cmd").sortable("enable"),$("#table_cmd th:first").width("50px");void 0==a.configuration.brkId||0>a.configuration.brkId||"eqpt"!=a.configuration.type&&"broker"!=a.configuration.type?($(".toDisable").addClass("disabled"),$(".typ-brk").hide(),$(".typ-std").show()):"broker"==a.configuration.type?
($(".toDisable").removeClass("disabled"),$(".typ-std").hide(),$(".typ-brk").show(),$("#mqtttopic").prop("readonly",!0),b="jMQTT_"+(a.name.replace(" ","_")||"jeedom"),$("input[name=rd_logupdate]").attr("data-l1key","log::level::"+b),$(".bt_plugin_conf_view_log").attr("data-log",b),$(".bt_plugin_conf_view_log").html('<i class="fa fa fa-file-text-o"></i> '+b),jeedom.config.load({configuration:$("#div_broker_log").getValues(".configKey")[0],plugin:"jMQTT",error:function(a){$("#div_alert").showAlert({message:a.message,
level:"danger"})},success:function(a){$("#div_broker_log").setValues(a,".configKey")}})):"eqpt"==a.configuration.type&&($(".toDisable").removeClass("disabled"),$(".typ-brk").hide(),$(".typ-std").show(),$("#mqtttopic").prop("readonly",!1));var f=$("#broker");f.empty();$.each(eqBrokers,function(a,b){f.append(new Option(b,a))});f.val(a.configuration.brkId)}function saveEqLogic(a){var c=$("#div_broker_log").getValues(".configKey")[0];$.isEmptyObject(c)||(a.loglevel=c);return a}
