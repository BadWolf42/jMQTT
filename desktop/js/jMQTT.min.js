...
$("#table_cmd").delegate(".listEquipementInfo","click",function(){var a=$(this);jeedom.cmd.getSelectModal({cmd:{type:"info"}},function(c){a.closest("tr").find(".cmdAttr[data-l1key=configuration][data-l2key="+a.data("input")+"]").atCaret("insert",c.human);modifyWithoutSave=!0})});$(".eqLogicAction[data-action=refreshPage]").on("click",refreshEqLogicPage);$("#bt_classic").on("click",refreshEqLogicPage);$("#bt_json").on("click",refreshEqLogicPage);$('a[href="#eqlogictab"]').on("click",function(){$("#menu-bar").hide()});
$('a[href="#commandtab"]').on("click",function(){$("#menu-bar").show()});1==getUrlVars("saveSuccessFull")&&($("#div_alert").showAlert({message:"{{Sauvegarde effectu\u00e9e avec succ\u00e8s}}",level:"success"}),history.replaceState(history.state,"",initPluginUrl(["saveSuccessFull"])));1==getUrlVars("removeSuccessFull")&&($("#div_alert").showAlert({message:"{{Suppression effectu\u00e9e avec succ\u00e8s}}",level:"success"}),history.replaceState(history.state,"",initPluginUrl(["removeSuccessFull"])));
...
c+' placeholder="{{Topic}}" readonly=true />';b+="</td><td>";b+='<textarea class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="value" style="height:65px;" '+c+' placeholder="{{Valeur}}" readonly=true />';b+="</td><td>";b+='<input class="cmdAttr form-control input-sm" data-l1key="unite" placeholder="{{Unit\u00e9}}"></td><td>';b+='<span><label class="checkbox-inline"><input type="checkbox" class="cmdAttr checkbox-inline" data-l1key="isHistorized" checked/>{{Historiser}}</label></span> ';
